...
describe('AbstractSchemaNode', () => {
  ...
  describe('Integration with MockToolExecutor', () => {
    it('should record tool invocations in mock executor', () => {
      const toolInvocationData: MCPToolInvocationData<typeof TestInputSchema> = {
        llmMetadata: {
          name: 'test-tool',
          description: 'A test tool',
      ...
      expect(lastCall?.llmMetadata.name).toBe('test-tool');
      expect(lastCall?.input).toEqual({ testField: 'test-value' });
    });

    it('should allow resetting mock executor between tests', () => {
      const toolInvocationData: MCPToolInvocationData<typeof TestInputSchema> = {
        llmMetadata: {
          name: 'test-tool',
          description: 'A test tool',
      ...
      expect(mockToolExecutor.getCallHistory()).toHaveLength(1);

      mockToolExecutor.reset();

      expect(mockToolExecutor.getCallHistory()).toHaveLength(0);
