...
const RESOURCES_DIR = path.resolve(PROJECT_ROOT, 'packages/mobile-web/resources');
...
async function downloadAndExtractLightningTypes(): Promise<void> {
  ...
  try {
    ...
    const mobileCapabilitiesSourceDir = path.resolve(
      extractedPackageDir,
      'dist/lightning/mobileCapabilities'
    );
