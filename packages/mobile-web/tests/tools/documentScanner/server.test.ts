...
import server from '../../../src/tools/documentScanner/server';
...
describe('Document Scanner Server', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should read type definitions when tool is called', async () => {
    ...
    await mockTransport.send({
      jsonrpc: '2.0',
      id: 1,
      method: 'sfmobile-web-document-scanner',
      params: {},
    });

    // Verify readFile was called with correct arguments
    expect(readFile).toHaveBeenCalledWith(
      expect.stringContaining('resources/documentScanner.d.ts'),
      'utf-8'
    );
  });
});
