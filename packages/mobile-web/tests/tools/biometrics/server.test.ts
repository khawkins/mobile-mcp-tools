...
import server from '../../../src/tools/biometrics/server';
...
describe('Biometrics Server', () => {
  ...
  it('should read type definitions when tool is called', async () => {
    ...
    await mockTransport.send({
      jsonrpc: '2.0',
      id: 1,
      method: 'sfmobile-web-biometrics',
      params: {},
    });

    // Verify readFile was called with correct arguments
    expect(readFile).toHaveBeenCalledWith(
      expect.stringContaining('resources/biometricsService.d.ts'),
      'utf-8'
    );
  });
});
