...
import server from '../../../src/tools/appReview/server';
...
describe('App Review Server', () => {
  ...
  it('should read type definitions when tool is called', async () => {
    ...
    await mockTransport.send({
      jsonrpc: '2.0',
      id: 1,
      method: 'sfmobile-web-app-review',
      params: {},
    });

    // Verify readFile was called with correct arguments
    expect(readFile).toHaveBeenCalledWith(
      expect.stringContaining('resources/appReviewService.d.ts'),
      'utf-8'
    );
  });
});
